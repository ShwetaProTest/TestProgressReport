#A Python script that reads input files, generates test case progress reports, and displays the results in a plot.t
# Developer: RHM
...
def format_axis(ax):
    ax.grid(axis='both', which='major', color='k', alpha=.12, linewidth=1.0, linestyle='--')
    ax.grid(axis='both', which='minor', color='k', alpha=.1, linewidth=0.5, linestyle='--')
    ax.tick_params(axis='y', which='major', length=3, labelsize=8)
    ax.tick_params(axis='x', which='major', length=7, labelsize=8, rotation=8)

def create_build_status_plot(date_counts_df, total_test_cases, build_name, build_status_report,new_labels,colors_dict,colors,report_pdf):
    ...
    output_bs_report = build_status_report + build_name + '_Buildstatus_TestCase.png'
    plt.savefig(output_bs_report, bbox_inches='tight', dpi=150)
    fig.tight_layout()

    # Convert the PNG image to PDF
    report_output = report_pdf
    output_bs_pdf = report_output + build_name + '_Buildstatus_TestCase.pdf'
    with open(output_bs_pdf, "wb") as pdf_file:
        pdf_file.write(img2pdf.convert(output_bs_report))

def product_comparison_report(df, names, new_labels, total_test_cases, comparison_report, colors,build_name,report_pdf):
    ...
    output_tc_image = comparison_report + 'Progress_Report_TestCase.png'
    plt.savefig(output_tc_image, bbox_inches='tight', dpi=150)
    ...
    output_tc_pdf = report_pdf + build_name + '_Progress_Report_TestCase.pdf' 
    with open(output_tc_pdf, "wb") as pdf_file:
        pdf_file.write(img2pdf.convert(output_tc_image))
    ####################################################################################################################
...
def test_case_report(build_input,testspecification_report,testcase_report,comparison_report,build_status_report,colors,current_datetime,report_pdf):
    print(f"\nscript3:")
    ...
    build_file_path = os.path.abspath(build_input)
    ...
    xmltoxl_file_path = os.path.abspath(os.path.join(testspecification_report, "TestSpecification_Testlink.xlsx"))
    ...
    for i, file_path in enumerate(build_files):
        ...
        #plot1
        create_build_status_plot(date_counts_df, total_test_cases, build_name, build_status_report,new_labels,colors_dict,colors,report_pdf)
    ...
    ####################################################################################################################
        ...
        output_tc_report = testcase_report + build_name + '_TestCase_plot.png'
        plt.savefig(output_tc_report, bbox_inches='tight',dpi=150)

        # Add the image to the PDF
        output_tc_pdf = report_pdf + build_name + '_TestCase_.pdf' 
        with open(output_tc_pdf, "wb") as pdf_file:
            pdf_file.write(img2pdf.convert(output_tc_report))
    ...
    ####################################################################################################################
    # plot3
    product_comparison_report(df, names, new_labels, total_test_cases, comparison_report, colors,build_name,report_pdf)
