...
def xml_xl(input_folder,xml_folder):
    ...
    for child in root.findall('testsuite'):
        for each in child.findall('.//testcase'):
            ...
            steps = each.find('steps')
            if steps is not None:
                last_step = steps.findall('step')[-1]
                step_number = last_step.find('step_number').text
            else:
                step_number = 0
            # for steps in each.findall('.//custom_fields'):
            #     for cs in steps.find(".//custom_field[name='Total test steps']"):
            #         value = cs.find('value').text if cs is not None and cs.find('value').text else '0'
            #         #last = cs[-1].text
            #         #first = cs[-4].text
            data.append({'Test_case_ID': test_id,'Test_case_title': Test_Case,'Version':version,'Estimated_exec_[min]':estimated_duration,'Total_steps':step_number,'Priority':imp})
...
def test_report(input_folder,build_input,build_report,current_datetime):
    print('Test report generation started')
    ...
    print("Iterate over the status and build files to extract the status of each test case present in the build")
    ...
    for files in Build_files:
        ...
        df_merged['Number of Not Run Steps'] = df_merged['Total_steps'].where(df_merged.iloc[:, 1].str.contains('Not Run'))

        # Rearrange the column names
        #df3 = df_merged[df_merged.columns[[5, 0, 2, 3, 4, 11, 6, 1, 7, 8, 9, 12, 10]]]
        df3 = df_merged[df_merged.columns[[5, 4, 1, 2, 3, 11, 6, 0, 7, 8, 9, 12, 10]]]
...
def Supporting_func(input_folder,xml_folder,build_input,build_report,current_datetime):
    ...
    print(f"Build Files have been created. File Generated in : {os.path.join(input_folder, 'Build_Input/')}")
